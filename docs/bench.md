...
| **0.1.0 基线（VEH）** | 4984 / 4556 / 5275 / 4617 / 4468 | **~2309** | 中位数 4617ms；同机 5 次无钉核（工具宿主限制）；与 0.0.3 基线 ±12% 内一致，佐证 0.0.6 的 2x 系噪音 |
| **0.1.0 M2（--trap=island）** | 4951 / 4821 / 5247 / 4426 / 4490（med 4821 ≈ VEH） | ~2400 | **诚实结论**：bench guest 的 syscall site 未全部通过岛页安全校验（syscall 后随 7 字节 `mov [rip+counter]` 不可迁移）→ auto/island 均整体回退 VEH，计时即 VEH 后端。岛页收益需「syscall 后接 ≤3 字节可迁移指令」的形态（musl `syscall; ret` 包裹层），当前验收集未覆盖；岛页机制本身已经 hello-musl（5 sites）/fs-exec 混合模式运行验证 |
