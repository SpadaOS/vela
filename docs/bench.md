...
| 0.0.6 | 8862 / 11122 | ~4400-5600 | ⚠ 本机负载噪音大（run1/run2 差 25%），显著高于 0.0.3 基线的结论**不成立**——需要 CI 隔离环境复测；0.0.4-0.0.5 热路径新增（fs heal rdfsbase、fork 分支判断）量级在 <100ns，不足以解释 2x |

## fork 往返（0.0.6 新增）

fork+exit+waitpid 全链路（fork-test guest）：本机实测单次 fork 快照
（~24 MB section 传递）+ 子进程重建 + waitpid 往返在毫秒级，体感即时。
脏页优化前不做精确基准（性能非目标）。
